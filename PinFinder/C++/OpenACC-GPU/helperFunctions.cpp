 ...
 *
 * NOTE: this version is identical to the procedural, C version of this project.
 * - This is due to the limitations of running OOP code on the GPU. When running code on
 * the device, we have the following limitations:
 *      1. The C++ "string" class cannot be used
 *      2. Class methods cannot be used as kernel functions
 * Because of these limitations, writing a C++ version of this program resulted in code that had
 * minimal changes from the C version. As a result of this low benefit for using a C++ solution,
 * we decided to only create a procedural solution.
 */
