...
#include "helperFunctions.h" // for PIN_DIGITS

// declare that these functions should be run on the device, not the host
#pragma omp declare target(LEFTROTATE, uint8ToHexStr, hashUsingMD5)
...
void hashUsingMD5(char *initial_msgStr, char *outputHash)
{
    ...
    // msg_len == ((((((PIN_DIGITS - 1) + 8) / 64) + 1) * 64) - 8) + 64;
