/** Compute/draw the Mandelbrot set using TSGL and CUDA
 *
 ...
 * The library does not support using CUDA to draw on the displayed canvas from the device.
 */

#include <cstdio>  // C-style I/O
#include <thrust/complex.h> // complex<T> -- std functions for gpu usage
#include <cstdlib> // abs()
...
{
    thrust::complex<long double> originalComplex(x, y);
    thrust::complex<long double> comp(x, y);
    int count = 0;
    while (abs(comp) < 2.0 && count < MAX_REPS)
    {
