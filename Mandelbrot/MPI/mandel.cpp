 ...
 * 
 * NOTE: Drawing on the canvas using the TSGL library must be done on a process
 * running on THIS DEVICE. The library does not support cores/processes belonging to a
 * different device writing to this device's display.
 */
...
#include <mpi.h>                // MPI
#include "parallelLoopChunks.h" // getChunkStartStopValues()
