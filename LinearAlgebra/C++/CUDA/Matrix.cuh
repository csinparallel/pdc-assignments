...
/** checkErr
 * a function for checking cuda errors and
 * printing details about such errors to the console
 */
inline void checkErr(cudaError_t err, const char *msg)
{
	if (err != cudaSuccess)
	{
		fprintf(stderr, "\n*** %s (error code %d: '%s')!\n\n",
...
inline Matrix<Item> Matrix<Item>::operator+(const Matrix<Item> &mat2) const
{
	...
	/* HINT: to transfer a Matrix's data to the device, you need to:
		- allocate space for the object and its array member (matrix.myMatrix)
		- copy the object, its array, and the pointer between the two
	*/
